...
func registerSubmit(c *gin.Context) {
	...
	if keyEnabled() {
		if db.QueryRow("SELECT 1 FROM beta_keys WHERE key_md5 = ?", c.PostForm("key")).
			Scan(new(int)) == sql.ErrNoRows {
			registerResp(c, errorMessage{T(c, "This Key is not valid!")})
			return
		}
	}
	
	//if db.QueryRow("SELECT 1 FROM beta_keys WHERE key_md5 = ? AND allowed = 1", c.PostFrom("key")).
	//	Scan(new(int)) != sql.ErrNoRows {
	//	registerResp(c, errorMessage{T(c, "This Key is not valid.")})
	//	return
	//}
...
func keyEnabled() bool {
	var enabled bool
	db.QueryRow("SELECT value_int FROM bancho_settings WHERE name = 'usekeys'").Scan(&enabled)
	return enabled
}
